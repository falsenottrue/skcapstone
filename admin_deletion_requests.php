...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'Approve') {
        $stmt = $conn->prepare("SELECT login_id FROM account_deletion_requests WHERE request_id = ?");
        ...
        if ($login_id) {
            ...
            try {
                $del = $conn->prepare("DELETE FROM login WHERE login_id = ?");
                $del->bind_param("i", $login_id);
                $del->execute();
                $del->close();

                $upd = $conn->prepare("UPDATE account_deletion_requests SET status = 'Approved', admin_comment = ?, reviewed_at = NOW() WHERE request_id = ?");
                $upd->bind_param("si", $admin_comment, $request_id);
                $upd->execute();
                $upd->close();

                $conn->commit();
                echo "<script>alert('User account deleted and request approved.'); window.location.href='admin_deletion_requests.php';</script>";
            } catch (Exception $e) {
                $conn->rollback();
                echo "<script>alert('Error processing deletion: ".$e->getMessage()."'); window.location.href='admin_deletion_requests.php';</script>";
            }
        }
    } elseif ($action === 'Deny') {
        $upd = $conn->prepare("UPDATE account_deletion_requests SET status = 'Denied', admin_comment = ?, reviewed_at = NOW() WHERE request_id = ?");
        $upd->bind_param("si", $admin_comment, $request_id);
        if ($upd->execute()) {
            echo "<script>alert('Deletion request denied.'); window.location.href='admin_deletion_requests.php';</script>";
        } else {
            echo "<script>alert('Failed to update request.'); window.location.href='admin_deletion_requests.php';</script>";
        }
        $upd->close();
    }
}

$requests = $conn->query("SELECT r.request_id, r.login_id, r.request_date, r.status, l.usernm, l.email FROM account_deletion_requests r JOIN login l ON r.login_id = l.login_id WHERE r.status = 'Pending'");
$approved = $conn->query("SELECT r.request_id, r.login_id, r.request_date, r.status, r.reviewed_at, l.usernm, l.email, r.admin_comment FROM account_deletion_requests r JOIN login l ON r.login_id = l.login_id WHERE r.status = 'Approved'");
$denied = $conn->query("SELECT r.request_id, r.login_id, r.request_date, r.status, r.reviewed_at, l.usernm, l.email, r.admin_comment FROM account_deletion_requests r JOIN login l ON r.login_id = l.login_id WHERE r.status = 'Denied'");
?>
      ...
      <main role="main" class="main-content">
        ...
        >
          ...
          <div class="card shadow-sm mb-4">
            ...
            <h4 class="mb-0">Pending Deletion Requests</h4>
            </div>
            <div class="card-body">
            ...
            <div class="table-responsive">
                ...
                <tbody class="text-center">
                    ...
                    <td>
                        ...
                        <textarea name="admin_comment" class="form-control mb-2" rows="2" placeholder="Add comment (optional)"></textarea>
                        <div class="d-flex justify-content-center gap-2">
                            <button type="submit" name="action" value="Approve" class="btn btn-success btn-sm" onclick="return confirm('Approve and delete this account?')">Approve</button>
                            <button type="submit" name="action" value="Deny" class="btn btn-danger btn-sm" onclick="return confirm('Deny this request?')">Deny</button>
                        </div>
                        </form>
                    </td>
                    </tr>
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
            <div class="table-responsive">
                <table id="approvedTable" class="table table-bordered table-hover align-middle">
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
            <div class="table-responsive">
                <table id="deniedTable" class="table table-bordered table-hover align-middle">
                <thead class="table-secondary">
                    <tr>
                    <th>Request ID</th>
                    <th>User</th>
                ...
                <tbody class="table-secondary">
                    <?php while ($row = $denied->fetch_assoc()): ?>
                    <tr>
                    <td><?= $row['request_id'] ?></td>
                    <td><?= htmlspecialchars($row['usernm']) ?></td>
                    <td><?= htmlspecialchars($row['email']) ?></td>
